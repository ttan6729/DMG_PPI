...
class DenseAttentionModule(torch.nn.Module):
    ...
    def setup_weights(self):
        self.weight_matrix = torch.nn.Parameter(torch.Tensor(self.hidden_dim , self.hidden_dim ))
...
class TensorNetworkModule(torch.nn.Module):
    ...
    def __init__(self, input_dim,tensor_neuron=16):
        """
        :param args: Arguments object.
        """
        super(TensorNetworkModule, self).__init__()
        self.input_dim = input_dim
        self.tensor_neuron = tensor_neuron
        self.setup_weights()
        self.init_parameters()
    ...
    def setup_weights(self):
        ...
        """
        self.weight_matrix = torch.nn.Parameter(
            torch.Tensor( self.input_dim, self.input_dim, tensor_neuron ) )
        self.weight_matrix_block = torch.nn.Parameter(
            torch.Tensor( self.tensor_neuron, 2 * self.input_dim) )
        self.bias = torch.nn.Parameter(torch.Tensor(self.tensor_neuron, 1))
    ...
    def forward(self, embedding_1, embedding_2):
        ...
        scoring = torch.matmul(
            embedding_1, self.weight_matrix.view(self.input_dim, -1)
        )
        scoring = scoring.view(batch_size, self.input_dim -1).permute([0, 2, 1])
        scoring = torch.matmul(
            scoring, embedding_2.view(batch_size, self.input_dim, 1)
        ).view(batch_size, -1)
...
class DiffPool(torch.nn.Module):
    def __init__(self, input_dim, num_nodes=10, num_layers=4, hidden=16, ratio=0.25):
        super(DiffPool, self).__init__()
        #self.args = args
        self.att = DenseAttentionModule(hidden)

        num_nodes = ceil(ratio * num_nodes)
        self.embed_block1 = Block(input_dim, hidden, hidden)
        self.pool_block1 = Block(input_dim, hidden, num_nodes)
        ...
        self.lin2 = Linear(hidden, input_dim)
