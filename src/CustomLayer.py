...
class MixGINConv(MessagePassing):
    ...
    def __init__(self, input_dim, output_dim, eps: float = 0., train_eps: bool = False, layers=3,bias=True,**kwargs):
        ...
        self.layers = layers
        if bias:
            self.bias = Parameter(torch.empty((layers) * output_dim))
    ...
    def forward(
        ...
        # propagate_type: (x: OptPairTensor)
        outs = [self.lins[0](x[0])]

        for i in range(self.layers-1):
            x = self.propagate(edge_index, x=x, size=size)
            outs.append(self.lins[i+1](x))
            if isinstance(x, Tensor):
                x = (x, x)
